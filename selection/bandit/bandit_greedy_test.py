...
def generate_data(beta,
    ...
    if dimX == 2:
        ...
        A = np.append(X, D)
    Y = (A.dot(beta) + (np.random.standard_normal(n)) * sigma) #Normal Arms
    # Y = (A.dot(beta) + (np.random.standard_t(df,n)) * sigma) #t-distribution Arms
...
def bandit(A_0, Y_0, d, beta, sigma=1., randomizer_scale=0.5):
    ...
    w=np.random.multivariate_normal(np.zeros(beta.shape[0]),randomizer_scale*sigma*np.identity(beta.shape[0]),1)
    ...
    for u in range(K - 1):
        target_linear[u, :] = contexts[:, nch[u]] - contexts[:, ch]
    ...
    lower = target_1 - (1.65 * sd)  # Normal CI 90%
    upper = target_1 + (1.65 * sd)
    # lower = target_1 - (t.ppf(0.9,df=df) * sd) #t-distribution CI 90%
    # upper = target_1 + (t.ppf(0.9,df=df) * sd)

    return ((lower < beta)*(beta < upper)), A_1, Y_1, (target_1-beta)

def test(beta = np.array([0,0,-10,-10,10,10]), rho=0., equicorrelated=False, T=100, sigma=0.000001):
    ...
    for i in range(beta.shape[0]-2):
        ...
        if i == 0:
            ...
            A_0 = np.vstack((A_0,D_0))
            Y_0 = np.append(Y_0,y_0)
    cov = np.zeros((T, beta.shape[0]))
    bias = np.zeros((T, beta.shape[0]))
    for u in range(T):
        cov[u,:], A_1, Y_1, bias[u, :] = bandit(A_0, Y_0, d=2, beta=beta, sigma=sigma)
        A_0 = A_1
        Y_0 = Y_1
